...
## --- Define sample properties - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    smpl.Name          = ("C29r", "KPB", "C29n", "C28r") # Et cetera
    smpl.Age          .= [ 66.38,  66.021,  65.70,  64.862,] # Measured ages
    smpl.Age_sigma    .= [   0.050,    0.081,   0.050,    0.050,] # Measured 1-σ uncertainties
    smpl.Height       .= [ 1885,  1925 ,1955,2045,] # Depths below surface should be negative
    smpl.Height_sigma .= fill(1.0, nSamples) # Usually assume little or no sample height uncertainty
    ...
    smpl.Height_Unit = " DGNSS Elevation (m)" # Unit of measurement for Height and Height_sigma
...
## --- Optional: Stratigraphic model including hiatuses - - - - - - - - - - - -
    ...
    @time (mdl, agedist, lldist) = StratMetropolis(smpl, config); sleep(0.5)
